...
void main() {
  group('FilteredTodos', () {
    ...
    testWidgets('should show empty list when state is TodosLoaded with no Todo',
        (tester) async {
      ...
      await tester.pumpAndSettle();
      Finder todosListFinder = find.byKey(ArchSampleKeys.todoList);
      expect(todosListFinder, findsOneWidget);
      expect(
          (todosListFinder.evaluate().first.widget as ListView)
              .semanticChildCount,
          0);
    });
    ...
    testWidgets('should add OnCheckboxChanged when checkbox is tapped',
        (WidgetTester tester) async {
    ...
    testWidgets('should navigate to DetailScreen when Todo Tapped',
        (WidgetTester tester) async {
      when(todosBloc.state)
          .thenAnswer((_) => TodosLoaded([Todo('wash car', id: '0')]));
      when(filteredTodosBloc.state).thenAnswer(
